...
class validator
{
	...
	public function validateDates($from, $until)
    {
        $arrfrom = explode("-",$from);
        $arruntil = explode("-",$until);
        if($arruntil[0] < $arrfrom[0])
        {
            return false;
        }
        else if($arruntil[0] > $arrfrom[0])
        {
            return true;
        }
        else
        {
            if($arrfrom[1] > $arruntil[1]) return false;
            else if($arrfrom[1] < $arruntil[1]) return true;
            else
            {
                if($arruntil[2]<$arrfrom[2]) return false;
                return true;
            }
        }
    }
